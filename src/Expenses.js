    ...
    return(
        <motion.div 
            ...
            <div className="h-screen bg-gradient-to-r from-purple-600 to-pink-400 justify-center">
                ...
                animate= {{ x: 0 }}
                transition={{ delay: 2.5, type: "spring", stiffness: 150 }}>Expenses panel</motion.h1>
                <motion.button 
                    initial={{ x:'100vw' }} 
                    animate={{ x: 0 }}
                    transition={{ type: "spring", delay: 2.2, stiffness: 150 }}
                    className="mt-9 rounded-full border-rose-600 text-purple-400 bg-white p-3 w-36 hover:bg-purple-500 hover:text-white transition-all"><Link to="/invoice" element={<Invoice />}>Proceed</Link></motion.button>
                </div>
            </div>
        </motion.div>
    );
